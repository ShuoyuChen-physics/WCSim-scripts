...
Date: 2025-03-19 16:06:35
LastEditors: Shuoyu Chen shuoyuchen.physics@gmail.com
LastEditTime: 2025-03-19 16:06:35
FilePath: /schen/workspace/HKFDML/WCSim-scripts/merge_h5.py
Description: 
'''
import argparse
import h5py
    ...
    for fn in config.input_files[1:]:
        ...
        for k in keys:
            ...
            if shapes[k][1:] != list(f[k].shape[1:]):
                raise ValueError(f"Array {k} in {f.filename} has shape {f[k].shape} which is incompatible with extending previous files shape {shapes[k]}.")
    for k in attr_keys:
